...
import { backupCsvToDrive } from './services/driveBackupService';
...
const App = () => {
  ...
  const brokersRef = useRef<DataBroker[]>(brokers);
  ...
  const triggerBackup = async (data: DataBroker[], showToast = true) => {
    try {
      setIsBackingUp(true);
      const csv = exportBrokersToCsv(data);
      await backupCsvToDrive(csv);
      if (showToast) {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Drive backup failed. Check your connection.';
      if (showToast) {
        toast.error(message);
      }
    } finally {
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      void triggerBackup(brokersRef.current, false);
    };
      ...
      <header className="border-b border-border bg-surface/60">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 py-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-content hover:border-accent/70 disabled:opacity-60"
              onClick={() => triggerBackup(brokers)}
